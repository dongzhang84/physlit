"""03_decay Agent 1 — NON-CANONICAL content-axis stage-verdict resolver.

Resolves Stage 1 / Stage 2 / Stage 3 verdict-level disagreements
between the two content judges using the global Agent 1 prompt
(``prompts/agent1_content_resolver.md``) with 03_decay criteria.
Mirrors ``scripts/run_agent1_v0_3.py`` but pointed at the
03_decay results tree and the locked 03_decay criteria.

**Not part of the prereg envelope** — the human audit is canonical.
Verdicts to ``results/<model>/03_decay/content_resolved/``.

For Stage 3 the Agent 1 verdict is the OVERALL Stage 3 PASS / FAIL
(derived in production from all-scenarios-PASS). Per-scenario
resolution lives in ``run_agent2_03_decay.py``.

Resolver model: ``gemini-3.1-pro-preview``. Run with the sandbox
disabled (Bash sandbox blocks the google-genai endpoint).

Usage: ``uv run python scripts/run_agent1_03_decay.py``
"""
...
def main() -> int:
    ...
    print("=== 03_decay Agent 1 — non-canonical content-axis verdict resolver ===")
    ...
    print("Agent 1 verdicts saved under results/<model>/03_decay/content_resolved/.")
    return 0
