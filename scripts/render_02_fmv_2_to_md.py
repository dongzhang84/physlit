"""Render every 02_fmv.2 treatment-arm trial JSON into a companion .md.

For each ``results/<model-id>/02_fmv_2/<stage>/trial_<N>_t<T>.json`` emit
a sibling ``trial_<N>_t<T>.md`` containing the verbatim prompt, the
verbatim response, the trial metadata, and (when present) both judges'
verdicts pulled from ``results/<model-id>/02_fmv_2/judgments/``.

Parallel to ``scripts/render_02_fmv_to_md.py`` — this one targets the
``02_fmv_2`` framework directory and skips all non-stage subdirs (the
treatment tree has ``judgments``, ``structural``, ``content_resolved``,
and ``structural_resolved`` siblings to the four stage dirs).

The .md files are derivative — never edit them by hand; re-run the
script to regenerate. JSON is the source of truth and is untouched.

Usage: ``uv run python scripts/render_02_fmv_2_to_md.py``
"""
