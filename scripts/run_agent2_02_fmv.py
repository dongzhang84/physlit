"""02_fmv.1 Agent 2 — NON-CANONICAL structural-axis disagree resolver.

Runs an LLM resolver over the structural-axis dual-judge disagreement
cases. For each case the resolver reads the Stage 1 rule set, Stage 2
context, both structural judges' verdicts, and the frozen structural
criteria, and returns PASS / FAIL.

**This is NOT part of the prereg-02_fmv.1-locked envelope.** The §1
"Out of scope" clause commits 02_fmv.1 to human-audit-only resolution
of structural disagreements. Agent 2 here is a side analysis only — to
be compared against the human audit. Agent 2 verdicts do **not** feed
P1 / P2; the human audit does. Output is written to a separate
directory and clearly labelled.

Resolver model: ``gemini-3.1-pro-preview`` — cross-vendor against the
two structural judges (Claude + GPT). Runs via the google-genai SDK;
the Bash sandbox blocks that endpoint, so run with the sandbox
disabled.

Usage:
    uv run python scripts/run_agent2_02_fmv.py
"""
...
def _structural_verdicts(model: str) -> dict[tuple[int, str], dict[str, Any]]:
    """{(trial_index, judge_family): parsed_verdict} from the structural dir."""
...
def main() -> int:
    _load_dotenv()
    tmpl = PromptTemplate(FRAMEWORK_DIR / "prompts" / "agent2_structural_resolver.md")
    criteria = (FRAMEWORK_DIR / "structural_criteria.md").read_text()
    observations = (FRAMEWORK_DIR / "observations.md").read_text()

    # Collect the structural-axis disagree cases.
    ...
    print("=== 02_fmv.1 Agent 2 — non-canonical structural-axis resolver ===")
    ...
    for model, trial in cases:
        ...
        out_dir = RESULTS / model / FRAMEWORK_ID / "structural_resolved"
    ...
    print("Agent 2 verdicts saved under results/<model>/02_fmv/structural_resolved/.")
    print("Compare against the human audit once the structural worksheet is filled in.")
    return 0
