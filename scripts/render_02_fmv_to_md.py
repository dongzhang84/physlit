"""Render every 02_fmv production trial JSON into a companion .md file.

For each ``results/<model-id>/02_fmv/<stage>/trial_<N>_t<T>.json`` emit
...
verdicts pulled from ``results/<model-id>/02_fmv/judgments/``.

Parallel to ``scripts/render_trials_to_md.py`` (which is hardcoded to
01_aristotelian); this one handles 02_fmv, whose judgments live under
the framework-scoped ``02_fmv/judgments/`` directory. No existing code
is modified.

The .md files are derivative — never edit them by hand; re-run the
script to regenerate. The JSON files are the source of truth and are
untouched.

Usage: ``uv run python scripts/render_02_fmv_to_md.py``
"""
...
def main() -> int:
    ...
    for model_dir in sorted(RESULTS.iterdir()):
        ...
        for stage_dir in sorted(framework_dir.iterdir()):
            if not stage_dir.is_dir() or stage_dir.name in ("judgments", "content_resolved"):
                continue
