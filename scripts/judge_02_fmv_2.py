"""02_fmv.2 dual-judge orchestrator — content axis + structural axis.

Judges the **treatment arm** of the axiomatisation control experiment
(`predictions/02_fmv_2_prereg.md`). For every treatment trial under
`results/<model>/02_fmv_2/`:

- **Content axis** — Stage 1/2/3 responses scored by the two content
  judges (Claude + GPT-5.5) with the `02_fmv` judge prompts and
  criteria, frozen at `prereg-02_fmv-locked`. Verdicts →
  `results/<model>/02_fmv_2/judgments/`. Stage 4 (meta) is **not**
  judged (prereg §1.4).
- **Structural axis** — the Stage 1 rule set (Stage 2 as context)
  scored by the two structural judges with the `02_fmv.1` structural
  criteria + judge prompt, frozen at `prereg-02_fmv.1-locked`.
...
`scripts/apply_02_fmv_2.py` after any audit.

Prompts and criteria are read from the shared `frameworks/02_fmv/`
directory; only the trial data comes from the `02_fmv_2/` tree. The
script modifies no existing code.

Usage:
    uv run python scripts/judge_02_fmv_2.py [--models <csv>] [--n-trials N]
...
TREATMENT_ID = "02_fmv_2"  # results subtree for the treatment arm
...
def _content_criteria() -> dict[str, str]:
    return {
        "ideal_induction_md": (SOURCE_DIR / "ideal_induction.md").read_text(),
        "pass_fail_criteria_md": (SOURCE_DIR / "pass_fail_criteria.md").read_text(),
        "prediction_tests_md": (SOURCE_DIR / "prediction_tests.md").read_text(),
    }
...
def _structural_prompt(s1: str, s2: str) -> str:
    return PromptTemplate(JUDGE_PROMPTS_DIR / "judge_structural.md").render(
        structural_criteria_md=(SOURCE_DIR / "structural_criteria.md").read_text(),
        observations_md=(SOURCE_DIR / "observations.md").read_text(),
        stage1_response=s1,
        stage2_response=s2,
    )
...
def aggregate(models: list[str], n_trials: int, judge_cost: float) -> None:
    ...
    o.append(
        "- Content axis judged with the `02_fmv` criteria + judge prompts; "
        "structural axis with the `02_fmv.1` criteria + judge prompt.\n"
    )
    if preliminary:
        o.append(
            f"- **PRELIMINARY** — {c_disagree} content + {s_disagree} structural "
            f"dual-judge disagreement(s) await human audit. Canonical P1 / P2 "
            f"and the treatment-vs-control comparison: `apply_02_fmv_2.py`.\n"
        )
    o.append("\n### Treatment-arm IRR\n")
    ...
    if not FINDINGS.exists():
        FINDINGS.write_text("# PhysLit 02_fmv.2 — Findings\n\n")
    ...
    print("Run scripts/apply_02_fmv_2.py for canonical P1 / P2 + treatment-vs-control.")
...
def main() -> int:
    ...
    print("=== PhysLit 02_fmv.2 dual-judge orchestrator (treatment arm) ===")
