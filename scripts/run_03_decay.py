"""03_decay production runner — Decay World x {Claude, GPT-5.5, Gemini 3.1 Pro}.

Usage:
    uv run python scripts/run_03_decay.py [options]
...
``predictions/03_decay_prereg.md``:
...
- Prompts are the **framework-specific** templates under
  ``frameworks/03_decay/prompts/`` — the Decay World banned-token set
  differs from both the v0.1 global ``prompts/`` and the ``02_fmv``
  templates, so neither is reused. In particular,
  ``stage1_induction.md`` carries the axiomatisation cue baked in by
  default (§1.2 of the prereg).
- Every Stage 3 prompt is built by parsing the
  ``frameworks/03_decay/prediction_tests.md`` answer file (single
  source of truth) — no hardcoded scenario constant.
- After the full trial set, R1(b) post-trial-set re-ping for Gemini.

This runner does not modify ``run_v0_1.py``, ``run_02_fmv.py``,
``run_02_fmv_2.py``, ``run_v0_3.py``, or any library code; the
Decay World experiment is independent of all prior PhysLit rounds.
...
# budget. Values mirror the 02_fmv runner.
STAGE_MAX_TOKENS = {
    "induction": 8192,
    "formulation": 8192,
...
# (observed: a Gemini Stage 4 call hung ~5 hours in the 02_fmv run).
# SIGALRM interrupts the blocking call; the _CallTimeout it raises is
# treated as transient and retried. SIGALRM is Unix / main-thread only
# — fine for this runner.
...
def run_one_trial_set(
    ...
    *,
    ...
    # Stage 1 — induction (axiomatisation cue baked in by default; see
    # prereg §1.2 and frameworks/03_decay/prompts/stage1_induction.md)
...
def _summarise_banned_signal(records: list[TrialRecord]) -> dict[str, int]:
    ...
    Note ``scan_for_banned`` uses the v0.1 banned list; for 03_decay it
    is only an approximate smoke-test signal, not the verdict. The
    Decay World's §3 list differs from v0.1's (overlapping on the
    energy / friction / force core, diverging on the physicist names
    and on dissipation / damping / viscosity), so the actual verdict
    must come from ``judge_stage1.md`` running against
    ``frameworks/03_decay/ideal_induction.md`` §3.
    """
    out: dict[str, int] = {}
    for r in records:
...
def _r1b_post_run_disclosure(
    ...
    if drift == "yes":
        block += (
            ...
            f"  deviation magnitude and decide whether the 03_decay\n"
