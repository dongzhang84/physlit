"""02_fmv.2 treatment-arm runner — F=mv World, axiomatisation control.

Usage:
    uv run python scripts/run_02_fmv_2.py [options]
...
Options:
    ...
    --output-mode {production,calibration,dryrun}
                            ``production``  → results/<model-version>/02_fmv_2/...
                            ``calibration`` → results/_calibration/<utc-ts>/...
                            ``dryrun``      → results/_dryrun/<utc-ts>/...
                            Default: production.

The script implements the **treatment arm** of the control experiment
pre-registered in ``predictions/02_fmv_2_prereg.md``:

- It is identical to ``run_02_fmv.py`` in every respect EXCEPT the
  Stage 1 induction prompt: this runner uses
  ``frameworks/02_fmv/prompts/stage1_induction_axiomatised.md`` (the
  ``02_fmv`` Stage 1 prompt plus the axiomatisation instruction).
  Stage 2/3/4 prompts, observations, models, N, temperature, and the
  fresh-session protocol are the same as ``02_fmv``.
- Output lands under the framework id ``02_fmv_2`` —
  ``results/<model-version>/02_fmv_2/<stage>/`` — kept fully separate
  from the frozen ``02_fmv`` control trials.
- Each trial creates a fresh API client and a new session UUID
  (``CLAUDE.md`` hard rule); stages run sequentially with no context
  reuse across stages.
- Every Stage 3 prompt is built from ``frameworks/02_fmv/
  prediction_tests.md`` (single source of truth).
- After the full trial set, R1(b) post-trial-set re-ping for Gemini.

This runner does not modify ``run_02_fmv.py`` or any library code; the
``02_fmv`` control arm and the ``02_fmv.2`` treatment arm are
independent.
"""
...
# Records and result directories carry the treatment-arm id ``02_fmv_2``;
# prompts, observations, and scenarios are sourced from the shared
# ``02_fmv`` framework directory (the prereg reuses them verbatim).
FRAMEWORK_ID = "02_fmv_2"
SOURCE_FRAMEWORK_DIR = REPO_ROOT / "frameworks" / "02_fmv"
...
# Per-stage max_tokens budgets — mirror the 02_fmv runner exactly.
STAGE_MAX_TOKENS = {
    "induction": 8192,
    "formulation": 8192,
    "prediction": 12288,
...
# Per-call wall-clock timeout (SIGALRM); matches run_02_fmv.py.
...
def run_one_trial_set(
    ...
    *,
    ...
    TrialRecords. Stage 1 uses the axiomatisation-treatment prompt; the
    other three stages are identical to the 02_fmv control runner."""
    records: list[TrialRecord] = []

    # Stage 1 — induction (TREATMENT: axiomatisation instruction added)
    s1_tmpl = PromptTemplate(PROMPTS_DIR / STAGE1_PROMPT_FILE)
    ...
    # Stage 2 — formulation (unchanged from 02_fmv)
    ...
    # Stage 3 — prediction (unchanged from 02_fmv)
    ...
    # Stage 4 — meta (unchanged from 02_fmv)
...
def _output_root_for_run(output_mode: str, runner: TestedModelRunner, run_timestamp: str) -> Path:
    """Per-mode output directory. ``save_trial`` appends
    ``/<framework_id>/<stage>/trial_<i>_t<temp>.json`` to the returned
    root — framework_id is ``02_fmv_2`` for this runner."""
...
def _r1b_post_run_disclosure(
    ...
    if drift == "yes":
        block += (
            ...
            f"  deviation magnitude and decide whether the 02_fmv.2\n"
...
def main() -> int:
    ...
    observations_list = _extract_observations_list(
        (SOURCE_FRAMEWORK_DIR / "observations.md").read_text()
    )
    ...
    print("=== PhysLit 02_fmv.2 runner (axiomatisation treatment arm) ===")
    print(f"  models:     {requested}")
    print(f"  n-trials:   {args.n_trials}")
    print(f"  output:     {args.output_mode} (framework id: {FRAMEWORK_ID})")
    print(f"  stage 1:    {STAGE1_PROMPT_FILE}")
