"""v0.3 treatment-arm runner — Aristotelian, axiomatisation control.

Usage:
    uv run python scripts/run_v0_3.py [options]
...
The treatment arm of the v0.3 control experiment pre-registered in
``predictions/v0_3_prereg.md``. It is identical to ``run_v0_1.py`` in
every respect EXCEPT the Stage 1 induction prompt: this runner uses
``frameworks/01_aristotelian/prompts/stage1_induction_axiomatised.md``
(the v0.1 global Stage 1 prompt with the axiomatisation paragraph
inserted; the paragraph is byte-for-byte identical to
...
the v0.1 control arm, the v0.3 treatment arm, and the 02_fmv.2
treatment arm are independent.
"""
...
# Per-stage max_tokens budgets — mirror the v0.1 / 02_fmv runners.
STAGE_MAX_TOKENS = {
    "induction": 8192,
    "formulation": 8192,
    "prediction": 12288,
...
# Per-call wall-clock timeout (SIGALRM); matches run_02_fmv_2.py.
...
def run_one_trial_set(
    ...
    *,
    ...
    """Run all four stages for one (runner, trial_index). Stage 1 uses
    the axiomatisation-treatment prompt under the Aristotelian framework
    dir; Stages 2-4 use the v0.1 global prompts."""
    records: list[TrialRecord] = []

    # Stage 1 — induction (TREATMENT: axiomatisation instruction added)
    s1_tmpl = PromptTemplate(FRAMEWORK_PROMPTS_DIR / STAGE1_PROMPT_FILE)
    ...
    # Stage 2 — formulation (v0.1 global prompt, unchanged)
    s2_tmpl = PromptTemplate(GLOBAL_PROMPTS_DIR / "stage2_formulation.md")
    ...
    # Stage 3 — prediction (v0.1 global prompt, unchanged)
    s3_tmpl = PromptTemplate(GLOBAL_PROMPTS_DIR / "stage3_prediction.md")
    ...
    # Stage 4 — meta (v0.1 global prompt, unchanged)
    s4_tmpl = PromptTemplate(GLOBAL_PROMPTS_DIR / "stage4_meta.md")
...
def _summarise_banned_signal(records: list[TrialRecord]) -> dict[str, int]:
    """Count banned-concept hits per stage (v0.1 banned list; smoke test)."""
    out: dict[str, int] = {}
    for r in records:
...
def _r1b_post_run_disclosure(
    ...
    """R1(b) post-trial-set re-ping for Gemini."""
    ...
    if drift == "yes":
        block += (
            ...
            f"  deviation magnitude and decide whether the v0.3\n"
...
def main() -> int:
    ...
    print("=== PhysLit v0.3 runner (Aristotelian axiomatisation treatment) ===")
    print(f"  models:     {requested}")
    print(f"  n-trials:   {args.n_trials}")
    print(f"  output:     {args.output_mode} (framework id: {FRAMEWORK_ID})")
    print(f"  stage 1:    {STAGE1_PROMPT_FILE} (framework-scoped)")
