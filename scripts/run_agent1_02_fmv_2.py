"""02_fmv.2 Agent 1 — NON-CANONICAL content-axis disagree resolver.

Runs an LLM resolver over the 02_fmv.2 treatment-arm content-axis
dual-judge disagreement cases (Stage 1 / 2 / 3). For each case the
resolver reads the tested response, both judges' verdicts + reasoning,
and the frozen 02_fmv criteria, and returns PASS / FAIL.

**This is NOT part of the prereg-02_fmv.2-locked envelope.** The §1.3
clause commits 02_fmv.2 to human-audit-only resolution of
disagreements. Agent 1 here is a side analysis only — to be compared
...
two content judges (Claude + GPT). google-genai SDK; the Bash sandbox
blocks that endpoint, so run with the sandbox disabled.
...
Usage:
    uv run python scripts/run_agent1_02_fmv_2.py
"""
...
TREATMENT_ID = "02_fmv_2"  # trials + judgments + resolved verdicts
    ...
    if stage == "prediction":
        ...
        for sc in parsed.get("scenarios") or []:
            ...
            lines.append(
                ...
                f"fails: {sc.get('failed_criterion') or '-'} | "
                f"{(sc.get('reasoning') or '').strip()}"
            )
        return "\n".join(lines)
...
def main() -> int:
    _load_dotenv()
    tmpl = PromptTemplate(SOURCE_DIR / "prompts" / "agent1_content_resolver.md")
    crit = {
    ...
    print("=== 02_fmv.2 Agent 1 — non-canonical content-axis resolver ===")
    ...
    print("Agent 1 verdicts saved under results/<model>/02_fmv_2/content_resolved/.")
    return 0
