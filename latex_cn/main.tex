...
% 中文支持依赖 ctex 包（TeX Live 或 MacTeX 默认携带）。
...
近三年里，大语言模型（LLM）能否进行结构化推理是 AI 研究的中心争议之一。Wei 等\citep{wei2022chain} 通过 Chain-of-Thought 提示让模型在多步算术与符号推理上的表现明显提升，Kojima 等\citep{kojima2022zero} 进一步表明 ``Let's think step by step'' 这类简单提示就能激发零样本推理能力，Huang 与 Chang\citep{huang2023reasoning} 在综述中梳理了围绕“LLM 是否真在做推理还是在记忆模板”的多种争论。
