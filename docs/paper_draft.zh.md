...
回到LLM物理这一问题上，前四段所讨论的现象指向同一个结构性缺口。推理一侧Mirzadeh等[7]的GSM-Symbolic显示前沿模型对题面扰动高度敏感，benchmark上的"答对"很大程度上来自符号匹配。做科学一侧的现有工作都把问题放在一个被人类先验确立的理论框架内，对框架本身的归纳没有独立完成。世界模型一侧评测的"物理"停留在感知与下一步预测，与物理学意义上的形式理论不在同一层面。LLM物理一侧2025年起的新一波基准在题源、评分规则、稳健性上各有改进，但范式仍然是给模型出题、看它答对多少。这一以"答对率"为核心的评测范式自身有两个结构性局限。其一，答对率原理上分不清"懂物理"与"训练数据里见过同类题"。基准越大、题量越多，分数自然越高，这种增长本身不蕴含认知能力的提升。其二，答对率不传达关于模型认知边界的任何信息。模型A得90%、模型B得91%，告诉读者的只是"B比A多对了一道"，并不告诉读者"B能做但A做不了的是哪一类认知工作"。当读者关心的是模型能否处理训练数据里没有过的物理场景，答对率不提供外推线索。
