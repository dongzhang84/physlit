...
我们提出的判别方式是把物理推理拆成归纳、系统化、应用、反思四个底层认知动作，给定一个明确的物理框架（含一组现象、一套判据、一组应用场景），让模型逐一执行这四个动作，对每个动作的输出做独立判定，最后看综合是否通过。这一拆分的依据是物理学训练的实际过程：物理学家的能力来自"观察归纳规律、把规律操作化、用规则预测新情境、回看推理识别错处"这套序列，任一动作缺失，物理推理都不完整。这一判别方式相对"答对一道道题"的进步在于：它分别测得四个独立能力的曲线，并能把模型在某一框架内的失败归因到具体认知动作。这也使得跨框架的失败模式有可能被比较，而不仅停留在分数的高低差异上。下面三节展开：§1.2解释四个认知动作各自测的是什么。§1.3解释三个框架的难度梯度设计。§1.4列出本文的具体贡献。

### 1.2 四个认知动作

物理学家的能力培养来自下面这套序列：

1. **归纳**（induction）：从一组现象里抽出能解释所有现象的规律。
2. **系统化**（formulation）：把规律写成可被第三方应用的操作性形式。
3. **应用**（prediction）：用规则对新情境做量化预测。
4. **反思**（reflection）：回看自己的推理，识别可能错的一步。

任一步缺失，物理推理都不完整。把这四步拆开测，可以分别看到模型在"理解现象""抽象规则""演绎应用""自我评估"上的能力曲线，而非把四种能力全压进一个综合分数里。

具体到本文的协议，四个动作对应四个阶段（Stage 1到Stage 4）。每个阶段是一个独立的API会话，跨阶段上下文复用被严格禁止。Stage 2看不到Stage 1的会话历史，只能看到Stage 1的最终响应作为输入文本。这一"严禁多轮上下文复用"的设定避免了"模型在跨阶段对话里把上一轮的错偷偷改掉"这类污染，让每一阶段的判断真正反映该阶段的独立能力。每个阶段有独立的判定准则，最后看四个阶段是否综合通过。判据细节见§2。

### 1.3 三个框架的难度梯度

只在一个框架上做完整评估，结论可能是该框架特有的产物。我们选了三个在认知压力上有明显差距的物理框架，构成难度梯度：

| 难度 | 框架 | 类型 | 主要施加的认知压力 |
|---|---|---|---|
| Easy | F=mv世界 | 反事实 | 单方程、单一物理领域，留下"能否放下牛顿"这一项 |
| Medium | 亚里士多德力学 | 历史 | 留下"能否暂停训练数据里那句'这是错的'"这一项 |
| Hard | Decay World | 反事实 | 跨四个物理领域、无底层物质、关闭所有耗散机制 |

这一梯度的设计逻辑是：单框架上的失败可能是该框架的偶然，跨三框架共性的失败更值得作为"模型物理素养"的特征看待。三个框架的详细介绍分别见§3、§4、§5。

### 1.4 本文贡献

本文在三个层面给出贡献。

**方法论层**：一套可复现、可审计的LLM物理素养评估流程。预注册预测以SHA-256加git tag锁定，每个阶段独立API会话防止跨阶段污染，双LLM裁判加25% IRR阈值触发人工audit，所有原始prompt、模型响应、判定记录、审计结论开放。三个框架完整可复现入口在三个对应的`prereg-<round>-locked`标签下。

**实验结论层**：三组实验综合内容通过率（composite content PASS）分别为9/15、5/15、0/15，与难度梯度方向一致。三家前沿模型（Claude Opus 4.7、GPT-5.5、Gemini 3.1 Pro）在三个框架上的失败模式有共性也有差异：定性方向上能力强（Decay World上60道定量题里0题方向错），定量比例上系统性套用训练数据里的标准物理公式（23题方向对但比例错）。具体见各framework章节。

**方法论发现层**：其一，Stage 4 over-claim比率在三个框架上稳定落在65到70之间，亚里士多德70%、F=mv 66.7%、Decay World 67%，表明模型的元认知校准与具体物理内容解耦，不会随框架变难而改善。其二，不同LLM裁判的可靠性不跨框架可迁移，且OpenAI裁判在Decay World上出现机制清晰的系统性失效（18个判断里16个引用虚构或误分类的禁词），为LLM-as-judge文献增加了一个可复现的failure mode数据点。
