...
v0.1 评测三个前沿模型：Claude Opus 4.7、GPT-5、Gemini 3。v1.0 扩展到包括 DeepSeek、Llama 等开源权重模型，以及推理优化模型。
...
### 8.1 阶段 1：v0.1（第 1–4 周）

**目标**：在一个现象集上发布端到端可工作的诊断，含完整预注册和复现包。
...
- 方法论文档完整到外部研究者可独立按图施工
- v0.1 结果**仅凭仓库**即可复现
- 至少**一篇**有实质内容的外部评论（任何方向都行）

### 8.2 阶段 2：v0.5（第 5–12 周）

**目标**：通过预印本和方法论深度建立可信度。

**交付物**：

- 5–7 个现象集，覆盖 A 和 B 两类
- 跨集合一致性分析
- 与 NewtonBench 等先前工作的显式对比
- 视觉化诊断报告，含能力矩阵
- 社区贡献现象集的标准化模板
- arXiv 预印本上线
...
### 8.3 阶段 3：v1.0（第 13–26 周）

**目标**：让 PhysLit 成为大模型认知评测领域被认可的标准。
...
- 至少 3 个社区贡献的现象集
- 完整学术论文
- 可选：NeurIPS 或 ICLR Datasets & Benchmarks Track 投稿

**判定阶段 3 是否成功的指标**：

- 至少 5 次外部学术引用
- 在 LLM 物理推理的学术讨论中被引用
- 至少一家主要 AI lab 用 PhysLit 框架评测过自家模型

---

## 9. 成功标准

每个阶段有明确的成功标准。**不达标触发项目复审，而不是延期**。复审有三个选项：修订计划、接受缩减范围、终止项目。

### 9.1 阶段 1 标准（第 4 周）

- 仓库公开，含宽松许可证
- 方法论文档完整自洽（外部研究者仅凭仓库即可执行协议）
- 预注册预测已 commit，时间戳确定，且自首次 commit 以来未修改
...
- arXiv 预印本上线
- 至少一次外部复现尝试
- 至少一次学术引用 / 正式博客讨论 / 同行批评

### 9.3 阶段 3 标准（第 26 周）

- 5 次外部学术引用
- 至少一家主要 AI lab 或公司用 PhysLit 框架评测过其模型
- 在 LLM 物理推理的学术讨论中被引用

任一阶段未达标，项目进入一周复审模式后再决定下一步。
...
- 方法论文档定稿
- 写明三 stage 的精确 pass/fail 判据
- 把三层测试协议写到操作级别
- 决定准确的 prompt 和指令
- **任何模型动数据之前 commit 预注册预测**
...
- 找至少一个外部人士 review（如有可能）
- 根据反馈打磨

**交付物**：方法论文档、预注册、初始结构的仓库。

### 第 2 周：第一个现象集

**周一–周二**：

- 写亚里士多德力学的 `observations.md`
- 确保现象描述不带理论加载
- 验证观察足以支撑预期的归纳推理

**周三**：

- 写 `ideal_induction.md` 描述理想归纳长什么样
- 写 `formulation_template.md`

**周四–周五**：

- 写 `prediction_tests.md` 含 5 个新场景
- 写 `pass_fail_criteria.md` 含明确二元判据
- 写 `meta_questions.md`

**周末**：

- 自审整个现象集
- 在某一个模型上**非正式跑一遍 prompt**，发现问题
...
- 实现 `claude_runner.py`（Anthropic API）
- 实现 `openai_runner.py`（OpenAI API）
- 实现 `gemini_runner.py`（Google API）
- 共享工具：prompt 拼装、响应记录、5 次试运行编排

**周三**：

- 对 Claude Opus 4.7 跑完整协议（每 stage 5 次）
- 记录所有响应
- 应用 pass/fail 判据
...
**交付物**：三次完整模型评测，含原始数据和判定。

### 第 4 周：分析、发表、发布

**周一**：

- 写 `v0_1_findings.md` 含诊断结论
- 评估每条预注册预测（确认 / 部分 / 反驳）
- 制作能力矩阵可视化
- 文档定稿
...
- 在 Twitter、Hacker News、相关 subreddit 宣发
- 参与早期讨论
- 收集反馈
